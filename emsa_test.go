package pss

import (
	"crypto/sha1"
	"fmt"
	"math/big"
	"testing"
)

func printBytes(name string, buf []byte) {
	fmt.Printf("%v:\n", name)
	for _, d := range buf {
		fmt.Printf("%x, ", d)
	}
	fmt.Printf("\n")
}

func compareBytes(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i, e := range a {
		if b[i] != e {
			return false
		}
	}
	return true
}

// Test vector from RSA lab: ftp://ftp.rsasecurity.com/pub/pkcs/pkcs-1/pkcs-1v2-1-vec.zip
func TestEMSAPSS(t *testing.T) {
	msg := []byte{0x85, 0x9e, 0xef, 0x2f, 0xd7, 0x8a, 0xca, 0x00, 0x30, 0x8b, 0xdc, 0x47, 0x11, 0x93, 0xbf, 0x55, 0xbf, 0x9d, 0x78, 0xdb, 0x8f, 0x8a, 0x67, 0x2b, 0x48, 0x46, 0x34, 0xf3, 0xc9, 0xc2, 0x6e, 0x64, 0x78, 0xae, 0x10, 0x26, 0x0f, 0xe0, 0xdd, 0x8c, 0x08, 0x2e, 0x53, 0xa5, 0x29, 0x3a, 0xf2, 0x17, 0x3c, 0xd5, 0x0c, 0x6d, 0x5d, 0x35, 0x4f, 0xeb, 0xf7, 0x8b, 0x26, 0x02, 0x1c, 0x25, 0xc0, 0x27, 0x12, 0xe7, 0x8c, 0xd4, 0x69, 0x4c, 0x9f, 0x46, 0x97, 0x77, 0xe4, 0x51, 0xe7, 0xf8, 0xe9, 0xe0, 0x4c, 0xd3, 0x73, 0x9c, 0x6b, 0xbf, 0xed, 0xae, 0x48, 0x7f, 0xb5, 0x56, 0x44, 0xe9, 0xca, 0x74, 0xff, 0x77, 0xa5, 0x3c, 0xb7, 0x29, 0x80, 0x2f, 0x6e, 0xd4, 0xa5, 0xff, 0xa8, 0xba, 0x15, 0x98, 0x90, 0xfc}
	salt := []byte{0xe3, 0xb5, 0xd5, 0xd0, 0x02, 0xc1, 0xbc, 0xe5, 0x0c, 0x2b, 0x65, 0xef, 0x88, 0xa1, 0x88, 0xd8, 0x3b, 0xce, 0x7e, 0x61}
	em := []byte{}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())

	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	encoded, err := emsaPSSEncode(hashed, 1023, salt, sha1.New())
	if err != nil {
		t.Errorf("Error: %v\n", err)
	}
	if !compareBytes(encoded, em) {
		t.Errorf("Bad encoding")
	}

	err = emsaPSSVerify(hashed, encoded, 1023, len(salt), sha1.New())
	if err != nil {
		t.Errorf("Bad verification")
	}
}

func fromBase16(base10 string) *big.Int {
	i := new(big.Int)
	i.SetString(base10, 16)
	return i
}
