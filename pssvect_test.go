package rsa

import (
	"crypto"
	"crypto/rand"
	. "crypto/rsa"
	"crypto/sha1"
	"fmt"
	"math/big"
	weakrand "math/rand"
	"testing"
)

func TestPSSSign_1_1_0(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("e7e8942720a877517273a356053ea2a1bc0c94aa72d55c6e86296b2dfc967948c0a72cbccca7eacb35706e09a1df55a1535bd9b3cc34160b3b6dcd3eda8e6443"),
			fromBase16("b69dca1cf7d4d7ec81e75b90fcca874abcde123fd2700180aa90479b6e48de8d67ed24f9f19d85ba275874f542cd20dc723e6963364a1f9425452b269a6799fd"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xde, 0xe9, 0x59, 0xc7, 0xe0, 0x64, 0x11, 0x36, 0x14, 0x20, 0xff, 0x80, 0x18, 0x5e, 0xd5, 0x7f, 0x3e, 0x67, 0x76, 0xaf}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_1_2_1(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("e7e8942720a877517273a356053ea2a1bc0c94aa72d55c6e86296b2dfc967948c0a72cbccca7eacb35706e09a1df55a1535bd9b3cc34160b3b6dcd3eda8e6443"),
			fromBase16("b69dca1cf7d4d7ec81e75b90fcca874abcde123fd2700180aa90479b6e48de8d67ed24f9f19d85ba275874f542cd20dc723e6963364a1f9425452b269a6799fd"),
		},
	}
	msg := []byte{0x85, 0x13, 0x84, 0xcd, 0xfe, 0x81, 0x9c, 0x22, 0xed, 0x6c, 0x4c, 0xcb, 0x30, 0xda, 0xeb, 0x5c, 0xf0, 0x59, 0xbc, 0x8e, 0x11, 0x66, 0xb7, 0xe3, 0x53, 0x0c, 0x4c, 0x23, 0x3e, 0x2b, 0x5f, 0x8f, 0x71, 0xa1, 0xcc, 0xa5, 0x82, 0xd4, 0x3e, 0xcc, 0x72, 0xb1, 0xbc, 0xa1, 0x6d, 0xfc, 0x70, 0x13, 0x22, 0x6b, 0x9e}
	sig := []byte{}
	salt := []byte{0xef, 0x28, 0x69, 0xfa, 0x40, 0xc3, 0x46, 0xcb, 0x18, 0x3d, 0xab, 0x3d, 0x7b, 0xff, 0xc9, 0x8f, 0xd5, 0x6d, 0xf4, 0x2d}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_1_3_2(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("e7e8942720a877517273a356053ea2a1bc0c94aa72d55c6e86296b2dfc967948c0a72cbccca7eacb35706e09a1df55a1535bd9b3cc34160b3b6dcd3eda8e6443"),
			fromBase16("b69dca1cf7d4d7ec81e75b90fcca874abcde123fd2700180aa90479b6e48de8d67ed24f9f19d85ba275874f542cd20dc723e6963364a1f9425452b269a6799fd"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x71, 0x0b, 0x9c, 0x47, 0x47, 0xd8, 0x00, 0xd4, 0xde, 0x87, 0xf1, 0x2a, 0xfd, 0xce, 0x6d, 0xf1, 0x81, 0x07, 0xcc, 0x77}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_1_4_3(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("e7e8942720a877517273a356053ea2a1bc0c94aa72d55c6e86296b2dfc967948c0a72cbccca7eacb35706e09a1df55a1535bd9b3cc34160b3b6dcd3eda8e6443"),
			fromBase16("b69dca1cf7d4d7ec81e75b90fcca874abcde123fd2700180aa90479b6e48de8d67ed24f9f19d85ba275874f542cd20dc723e6963364a1f9425452b269a6799fd"),
		},
	}
	msg := []byte{0xbc, 0x65, 0x67, 0x47, 0xfa, 0x9e, 0xaf, 0xb3, 0xf0}
	sig := []byte{}
	salt := []byte{0x05, 0x6f, 0x00, 0x98, 0x5d, 0xe1, 0x4d, 0x8e, 0xf5, 0xce, 0xa9, 0xe8, 0x2f, 0x8c, 0x27, 0xbe, 0xf7, 0x20, 0x33, 0x5e}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_1_5_4(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("e7e8942720a877517273a356053ea2a1bc0c94aa72d55c6e86296b2dfc967948c0a72cbccca7eacb35706e09a1df55a1535bd9b3cc34160b3b6dcd3eda8e6443"),
			fromBase16("b69dca1cf7d4d7ec81e75b90fcca874abcde123fd2700180aa90479b6e48de8d67ed24f9f19d85ba275874f542cd20dc723e6963364a1f9425452b269a6799fd"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x80, 0xe7, 0x0f, 0xf8, 0x6a, 0x08, 0xde, 0x3e, 0xc6, 0x09, 0x72, 0xb3, 0x9b, 0x4f, 0xbf, 0xdc, 0xea, 0x67, 0xae, 0x8e}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_2_1_5(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("016601e926a0f8c9e26ecab769ea65a5e7c52cc9e080ef519457c644da6891c5a104d3ea7955929a22e7c68a7af9fcad777c3ccc2b9e3d3650bce404399b7e59d1"),
			fromBase16("014eafa1d4d0184da7e31f877d1281ddda625664869e8379e67ad3b75eae74a580e9827abd6eb7a002cb5411f5266797768fb8e95ae40e3e8a01f35ff89e56c079"),
		},
	}
	msg := []byte{0xda, 0xba, 0x03, 0x20, 0x66, 0x26, 0x3f, 0xae, 0xdb, 0x65, 0x98, 0x48, 0x11, 0x52, 0x78, 0xa5, 0x2c, 0x44, 0xfa, 0xa3, 0xa7, 0x6f, 0x37, 0x51, 0x5e, 0xd3, 0x36, 0x32, 0x10, 0x72, 0xc4, 0x0a, 0x9d, 0x9b, 0x53, 0xbc, 0x05, 0x01, 0x40, 0x78, 0xad, 0xf5, 0x20, 0x87, 0x51, 0x46, 0xaa, 0xe7, 0x0f, 0xf0, 0x60, 0x22, 0x6d, 0xcb, 0x7b, 0x1f, 0x1f, 0xc2, 0x7e, 0x93, 0x60}
	sig := []byte{}
	salt := []byte{0x57, 0xbf, 0x16, 0x0b, 0xcb, 0x02, 0xbb, 0x1d, 0xc7, 0x28, 0x0c, 0xf0, 0x45, 0x85, 0x30, 0xb7, 0xd2, 0x83, 0x2f, 0xf7}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_2_2_6(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("016601e926a0f8c9e26ecab769ea65a5e7c52cc9e080ef519457c644da6891c5a104d3ea7955929a22e7c68a7af9fcad777c3ccc2b9e3d3650bce404399b7e59d1"),
			fromBase16("014eafa1d4d0184da7e31f877d1281ddda625664869e8379e67ad3b75eae74a580e9827abd6eb7a002cb5411f5266797768fb8e95ae40e3e8a01f35ff89e56c079"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x7f, 0x6d, 0xd3, 0x59, 0xe6, 0x04, 0xe6, 0x08, 0x70, 0xe8, 0x98, 0xe4, 0x7b, 0x19, 0xbf, 0x2e, 0x5a, 0x7b, 0x2a, 0x90}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_2_3_7(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("016601e926a0f8c9e26ecab769ea65a5e7c52cc9e080ef519457c644da6891c5a104d3ea7955929a22e7c68a7af9fcad777c3ccc2b9e3d3650bce404399b7e59d1"),
			fromBase16("014eafa1d4d0184da7e31f877d1281ddda625664869e8379e67ad3b75eae74a580e9827abd6eb7a002cb5411f5266797768fb8e95ae40e3e8a01f35ff89e56c079"),
		},
	}
	msg := []byte{0x52, 0xa1, 0xd9, 0x6c, 0x8a, 0xc3, 0x9e, 0x41, 0xe4, 0x55, 0x80, 0x98, 0x01, 0xb9, 0x27, 0xa5, 0xb4, 0x45, 0xc1, 0x0d, 0x90, 0x2a, 0x0d, 0xcd, 0x38, 0x50, 0xd2, 0x2a, 0x66, 0xd2, 0xbb, 0x07, 0x03, 0xe6, 0x7d, 0x58, 0x67, 0x11, 0x45, 0x95, 0xaa, 0xbf, 0x5a, 0x7a, 0xeb, 0x5a, 0x8f, 0x87, 0x03, 0x4b, 0xbb, 0x30, 0xe1, 0x3c, 0xfd, 0x48, 0x17, 0xa9, 0xbe, 0x76, 0x23, 0x00, 0x23, 0x60, 0x6d, 0x02, 0x86, 0xa3, 0xfa, 0xf8, 0xa4, 0xd2, 0x2b, 0x72, 0x8e, 0xc5, 0x18, 0x07, 0x9f, 0x9e, 0x64, 0x52, 0x6e, 0x3a, 0x0c, 0xc7, 0x94, 0x1a, 0xa3, 0x38, 0xc4, 0x37, 0x99, 0x7c, 0x68, 0x0c, 0xca, 0xc6, 0x7c, 0x66, 0xbf, 0xa1}
	sig := []byte{}
	salt := []byte{0xfc, 0xa8, 0x62, 0x06, 0x8b, 0xce, 0x22, 0x46, 0x72, 0x4b, 0x70, 0x8a, 0x05, 0x19, 0xda, 0x17, 0xe6, 0x48, 0x68, 0x8c}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_2_4_8(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("016601e926a0f8c9e26ecab769ea65a5e7c52cc9e080ef519457c644da6891c5a104d3ea7955929a22e7c68a7af9fcad777c3ccc2b9e3d3650bce404399b7e59d1"),
			fromBase16("014eafa1d4d0184da7e31f877d1281ddda625664869e8379e67ad3b75eae74a580e9827abd6eb7a002cb5411f5266797768fb8e95ae40e3e8a01f35ff89e56c079"),
		},
	}
	msg := []byte{0xa7, 0x18, 0x2c, 0x83, 0xac, 0x18, 0xbe, 0x65, 0x70, 0xa1, 0x06, 0xaa, 0x9d, 0x5c, 0x4e, 0x3d, 0xbb, 0xd4, 0xaf, 0xae, 0xb0, 0xc6, 0x0c, 0x4a, 0x23, 0xe1, 0x96, 0x9d, 0x79, 0xff}
	sig := []byte{}
	salt := []byte{0x80, 0x70, 0xef, 0x2d, 0xe9, 0x45, 0xc0, 0x23, 0x87, 0x68, 0x4b, 0xa0, 0xd3, 0x30, 0x96, 0x73, 0x22, 0x35, 0xd4, 0x40}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_2_5_9(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("016601e926a0f8c9e26ecab769ea65a5e7c52cc9e080ef519457c644da6891c5a104d3ea7955929a22e7c68a7af9fcad777c3ccc2b9e3d3650bce404399b7e59d1"),
			fromBase16("014eafa1d4d0184da7e31f877d1281ddda625664869e8379e67ad3b75eae74a580e9827abd6eb7a002cb5411f5266797768fb8e95ae40e3e8a01f35ff89e56c079"),
		},
	}
	msg := []byte{0x86, 0xa8, 0x3d, 0x4a, 0x72, 0xee, 0x93, 0x2a, 0x4f, 0x56, 0x30, 0xaf, 0x65, 0x79, 0xa3, 0x86, 0xb7, 0x8f, 0xe8, 0x89, 0x99, 0xe0, 0xab, 0xd2, 0xd4, 0x90, 0x34, 0xa4, 0xbf, 0xc8, 0x54, 0xdd, 0x94, 0xf1, 0x09, 0x4e, 0x2e, 0x8c, 0xd7, 0xa1, 0x79, 0xd1, 0x95, 0x88, 0xe4, 0xae, 0xfc, 0x1b, 0x1b, 0xd2, 0x5e, 0x95, 0xe3, 0xdd, 0x46, 0x1f}
	sig := []byte{}
	salt := []byte{0x17, 0x63, 0x9a, 0x4e, 0x88, 0xd7, 0x22, 0xc4, 0xfc, 0xa2, 0x4d, 0x07, 0x9a, 0x8b, 0x29, 0xc3, 0x24, 0x33, 0xb0, 0xc9}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_3_1_10(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("01bd36e18ece4b0fdb2e9c9d548bd1a7d6e2c21c6fdc35074a1d05b1c6c8b3d558ea2639c9a9a421680169317252558bd148ad215aac550e2dcf12a82d0ebfe853"),
			fromBase16("01b1b656ad86d8e19d5dc86292b3a192fdf6e0dd37877bad14822fa00190cab265f90d3f02057b6f54d6ecb14491e5adeacebc48bf0ebd2a2ad26d402e54f61651"),
		},
	}
	msg := []byte{0x59, 0x4b, 0x37, 0x33, 0x3b, 0xbb, 0x2c, 0x84, 0x52, 0x4a, 0x87, 0xc1, 0xa0, 0x1f, 0x75, 0xfc, 0xec, 0x0e, 0x32, 0x56, 0xf1, 0x08, 0xe3, 0x8d, 0xca, 0x36, 0xd7, 0x0d, 0x00, 0x57}
	sig := []byte{}
	salt := []byte{0xf3, 0x1a, 0xd6, 0xc8, 0xcf, 0x89, 0xdf, 0x78, 0xed, 0x77, 0xfe, 0xac, 0xbc, 0xc2, 0xf8, 0xb0, 0xa8, 0xe4, 0xcf, 0xaa}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_3_2_11(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("01bd36e18ece4b0fdb2e9c9d548bd1a7d6e2c21c6fdc35074a1d05b1c6c8b3d558ea2639c9a9a421680169317252558bd148ad215aac550e2dcf12a82d0ebfe853"),
			fromBase16("01b1b656ad86d8e19d5dc86292b3a192fdf6e0dd37877bad14822fa00190cab265f90d3f02057b6f54d6ecb14491e5adeacebc48bf0ebd2a2ad26d402e54f61651"),
		},
	}
	msg := []byte{0x8b, 0x76, 0x95, 0x28, 0x88, 0x4a, 0x0d, 0x1f, 0xfd, 0x09, 0x0c, 0xf1, 0x02, 0x99, 0x3e, 0x79, 0x6d, 0xad, 0xcf, 0xbd, 0xdd, 0x38, 0xe4, 0x4f, 0xf6, 0x32, 0x4c, 0xa4, 0x51}
	sig := []byte{}
	salt := []byte{0xfc, 0xf9, 0xf0, 0xe1, 0xf1, 0x99, 0xa3, 0xd1, 0xd0, 0xda, 0x68, 0x1c, 0x5b, 0x86, 0x06, 0xfc, 0x64, 0x29, 0x39, 0xf7}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_3_3_12(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("01bd36e18ece4b0fdb2e9c9d548bd1a7d6e2c21c6fdc35074a1d05b1c6c8b3d558ea2639c9a9a421680169317252558bd148ad215aac550e2dcf12a82d0ebfe853"),
			fromBase16("01b1b656ad86d8e19d5dc86292b3a192fdf6e0dd37877bad14822fa00190cab265f90d3f02057b6f54d6ecb14491e5adeacebc48bf0ebd2a2ad26d402e54f61651"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x98, 0x6e, 0x7c, 0x43, 0xdb, 0xb6, 0x71, 0xbd, 0x41, 0xb9, 0xa7, 0xf4, 0xb6, 0xaf, 0xc8, 0x0e, 0x80, 0x5f, 0x24, 0x23}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_3_4_13(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("01bd36e18ece4b0fdb2e9c9d548bd1a7d6e2c21c6fdc35074a1d05b1c6c8b3d558ea2639c9a9a421680169317252558bd148ad215aac550e2dcf12a82d0ebfe853"),
			fromBase16("01b1b656ad86d8e19d5dc86292b3a192fdf6e0dd37877bad14822fa00190cab265f90d3f02057b6f54d6ecb14491e5adeacebc48bf0ebd2a2ad26d402e54f61651"),
		},
	}
	msg := []byte{0x8f, 0xb4, 0x31, 0xf5, 0xee, 0x79, 0x2b, 0x6c, 0x2a, 0xc7, 0xdb, 0x53, 0xcc, 0x42, 0x86, 0x55, 0xae, 0xb3, 0x2d, 0x03, 0xf4, 0xe8, 0x89, 0xc5, 0xc2, 0x5d, 0xe6, 0x83, 0xc4, 0x61, 0xb5, 0x3a, 0xcf, 0x89, 0xf9, 0xf8, 0xd3, 0xaa, 0xbd, 0xf6, 0xb9, 0xf0, 0xc2, 0xa1, 0xde, 0x12, 0xe1, 0x5b, 0x49, 0xed, 0xb3, 0x91, 0x9a, 0x65, 0x2f, 0xe9, 0x49, 0x1c, 0x25, 0xa7, 0xfc, 0xe1, 0xf7, 0x22, 0xc2, 0x54, 0x36, 0x08, 0xb6, 0x9d, 0xc3, 0x75, 0xec}
	sig := []byte{}
	salt := []byte{0xf8, 0x31, 0x2d, 0x9c, 0x8e, 0xea, 0x13, 0xec, 0x0a, 0x4c, 0x7b, 0x98, 0x12, 0x0c, 0x87, 0x50, 0x90, 0x87, 0xc4, 0x78}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_3_5_14(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("01bd36e18ece4b0fdb2e9c9d548bd1a7d6e2c21c6fdc35074a1d05b1c6c8b3d558ea2639c9a9a421680169317252558bd148ad215aac550e2dcf12a82d0ebfe853"),
			fromBase16("01b1b656ad86d8e19d5dc86292b3a192fdf6e0dd37877bad14822fa00190cab265f90d3f02057b6f54d6ecb14491e5adeacebc48bf0ebd2a2ad26d402e54f61651"),
		},
	}
	msg := []byte{0xfe, 0xf4, 0x16, 0x1d, 0xfa, 0xaf, 0x9c, 0x52, 0x95, 0x05, 0x1d, 0xfc, 0x1f, 0xf3, 0x81, 0x0c, 0x8c, 0x9e, 0xc2, 0xe8, 0x66, 0xf7, 0x07, 0x54, 0x22, 0xc8, 0xec, 0x42, 0x16, 0xa9, 0xc4, 0xff, 0x49, 0x42, 0x7d, 0x48, 0x3c, 0xae, 0x10, 0xc8, 0x53, 0x4a, 0x41, 0xb2, 0xfd, 0x15, 0xfe, 0xe0, 0x69, 0x60, 0xec, 0x6f, 0xb3, 0xf7, 0xa7, 0xe9, 0x4a, 0x2f, 0x8a, 0x2e, 0x3e, 0x43, 0xdc, 0x4a, 0x40, 0x57, 0x6c, 0x30, 0x97, 0xac, 0x95, 0x3b, 0x1d, 0xe8, 0x6f, 0x0b, 0x4e, 0xd3, 0x6d, 0x64, 0x4f, 0x23, 0xae, 0x14, 0x42, 0x55, 0x29, 0x62, 0x24, 0x64, 0xca, 0x0c, 0xbf, 0x0b, 0x17, 0x41, 0x34, 0x72, 0x38, 0x15, 0x7f, 0xab, 0x59, 0xe4, 0xde, 0x55, 0x24, 0x09, 0x6d, 0x62, 0xba, 0xec, 0x63, 0xac, 0x64}
	sig := []byte{}
	salt := []byte{0x50, 0x32, 0x7e, 0xfe, 0xc6, 0x29, 0x2f, 0x98, 0x01, 0x9f, 0xc6, 0x7a, 0x2a, 0x66, 0x38, 0x56, 0x3e, 0x9b, 0x6e, 0x2d}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_4_1_15(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("029232336d2838945dba9dd7723f4e624a05f7375b927a87abe6a893a1658fd49f47f6c7b0fa596c65fa68a23f0ab432962d18d4343bd6fd671a5ea8d148413995"),
			fromBase16("020ef5efe7c5394aed2272f7e81a74f4c02d145894cb1b3cab23a9a0710a2afc7e3329acbb743d01f680c4d02afb4c8fde7e20930811bb2b995788b5e872c20bb1"),
		},
	}
	msg := []byte{0x9f, 0xb0, 0x3b, 0x82, 0x7c, 0x82, 0x17, 0xd9}
	sig := []byte{}
	salt := []byte{0xed, 0x7c, 0x98, 0xc9, 0x5f, 0x30, 0x97, 0x4f, 0xbe, 0x4f, 0xbd, 0xdc, 0xf0, 0xf2, 0x8d, 0x60, 0x21, 0xc0, 0xe9, 0x1d}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_4_2_16(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("029232336d2838945dba9dd7723f4e624a05f7375b927a87abe6a893a1658fd49f47f6c7b0fa596c65fa68a23f0ab432962d18d4343bd6fd671a5ea8d148413995"),
			fromBase16("020ef5efe7c5394aed2272f7e81a74f4c02d145894cb1b3cab23a9a0710a2afc7e3329acbb743d01f680c4d02afb4c8fde7e20930811bb2b995788b5e872c20bb1"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x22, 0xd7, 0x1d, 0x54, 0x36, 0x3a, 0x42, 0x17, 0xaa, 0x55, 0x11, 0x3f, 0x05, 0x9b, 0x33, 0x84, 0xe3, 0xe5, 0x7e, 0x44}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_4_3_17(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("029232336d2838945dba9dd7723f4e624a05f7375b927a87abe6a893a1658fd49f47f6c7b0fa596c65fa68a23f0ab432962d18d4343bd6fd671a5ea8d148413995"),
			fromBase16("020ef5efe7c5394aed2272f7e81a74f4c02d145894cb1b3cab23a9a0710a2afc7e3329acbb743d01f680c4d02afb4c8fde7e20930811bb2b995788b5e872c20bb1"),
		},
	}
	msg := []byte{0x28, 0x80, 0x62, 0xaf, 0xc0, 0x8f, 0xcd, 0xb7, 0xc5, 0xf8, 0x65, 0x0b, 0x29, 0x83, 0x73, 0x00, 0x46, 0x1d, 0xd5, 0x67, 0x6c, 0x17, 0xa2, 0x0a, 0x3c, 0x8f, 0xb5, 0x14, 0x89, 0x49, 0xe3, 0xf7, 0x3d, 0x66, 0xb3, 0xae, 0x82, 0xc7, 0x24, 0x0e, 0x27, 0xc5, 0xb3, 0xec, 0x43, 0x28, 0xee, 0x7d, 0x6d, 0xdf, 0x6a, 0x6a, 0x0c, 0x9b, 0x5b, 0x15, 0xbc, 0xda, 0x19, 0x6a, 0x9d, 0x0c, 0x76, 0xb1, 0x19, 0xd5, 0x34, 0xd8, 0x5a, 0xbd, 0x12, 0x39, 0x62, 0xd5, 0x83, 0xb7, 0x6c, 0xe9, 0xd1, 0x80, 0xbc, 0xe1, 0xca}
	sig := []byte{}
	salt := []byte{0x4a, 0xf8, 0x70, 0xfb, 0xc6, 0x51, 0x60, 0x12, 0xca, 0x91, 0x6c, 0x70, 0xba, 0x86, 0x2a, 0xc7, 0xe8, 0x24, 0x36, 0x17}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_4_4_18(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("029232336d2838945dba9dd7723f4e624a05f7375b927a87abe6a893a1658fd49f47f6c7b0fa596c65fa68a23f0ab432962d18d4343bd6fd671a5ea8d148413995"),
			fromBase16("020ef5efe7c5394aed2272f7e81a74f4c02d145894cb1b3cab23a9a0710a2afc7e3329acbb743d01f680c4d02afb4c8fde7e20930811bb2b995788b5e872c20bb1"),
		},
	}
	msg := []byte{0x6f, 0x4f, 0x9a, 0xb9, 0x50, 0x11, 0x99, 0xce, 0xf5, 0x5c, 0x6c, 0xf4, 0x08, 0xfe, 0x7b, 0x36, 0xc5, 0x57, 0xc4, 0x9d, 0x42, 0x0a, 0x47, 0x63, 0xd2, 0x46, 0x3c, 0x8a, 0xd4, 0x4b, 0x3c, 0xfc, 0x5b, 0xe2, 0x74, 0x2c, 0x0e, 0x7d, 0x9b, 0x0f, 0x66, 0x08, 0xf0, 0x8c, 0x7f, 0x47, 0xb6, 0x93, 0xee}
	sig := []byte{}
	salt := []byte{0x40, 0xd2, 0xe1, 0x80, 0xfa, 0xe1, 0xea, 0xc4, 0x39, 0xc1, 0x90, 0xb5, 0x6c, 0x2c, 0x0e, 0x14, 0xdd, 0xf9, 0xa2, 0x26}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_4_5_19(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("029232336d2838945dba9dd7723f4e624a05f7375b927a87abe6a893a1658fd49f47f6c7b0fa596c65fa68a23f0ab432962d18d4343bd6fd671a5ea8d148413995"),
			fromBase16("020ef5efe7c5394aed2272f7e81a74f4c02d145894cb1b3cab23a9a0710a2afc7e3329acbb743d01f680c4d02afb4c8fde7e20930811bb2b995788b5e872c20bb1"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x24, 0x97, 0xdc, 0x2b, 0x46, 0x15, 0xdf, 0xae, 0x5a, 0x66, 0x3d, 0x49, 0xff, 0xd5, 0x6b, 0xf7, 0xef, 0xc1, 0x13, 0x04}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_5_1_20(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("03f2f331f4142d4f24b43aa10279a89652d4e7537221a1a7b2a25deb551e5de9ac497411c227a94e45f91c2d1c13cc046cf4ce14e32d058734210d44a87ee1b73f"),
			fromBase16("034f090d73b55803030cf0361a5d8081bfb79f851523feac0a2124d08d4013ff08487771a870d0479dc0686c62f7718dfecf024b17c9267678059171339cc00839"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x08, 0x1b, 0x23, 0x3b, 0x43, 0x56, 0x77, 0x50, 0xbd, 0x6e, 0x78, 0xf3, 0x96, 0xa8, 0x8b, 0x9f, 0x6a, 0x44, 0x51, 0x51}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_5_2_21(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("03f2f331f4142d4f24b43aa10279a89652d4e7537221a1a7b2a25deb551e5de9ac497411c227a94e45f91c2d1c13cc046cf4ce14e32d058734210d44a87ee1b73f"),
			fromBase16("034f090d73b55803030cf0361a5d8081bfb79f851523feac0a2124d08d4013ff08487771a870d0479dc0686c62f7718dfecf024b17c9267678059171339cc00839"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xbd, 0x0c, 0xe1, 0x95, 0x49, 0xd0, 0x70, 0x01, 0x20, 0xcb, 0xe5, 0x10, 0x77, 0xdb, 0xbb, 0xb0, 0x0a, 0x8d, 0x8b, 0x09}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_5_3_22(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("03f2f331f4142d4f24b43aa10279a89652d4e7537221a1a7b2a25deb551e5de9ac497411c227a94e45f91c2d1c13cc046cf4ce14e32d058734210d44a87ee1b73f"),
			fromBase16("034f090d73b55803030cf0361a5d8081bfb79f851523feac0a2124d08d4013ff08487771a870d0479dc0686c62f7718dfecf024b17c9267678059171339cc00839"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x81, 0x57, 0x79, 0xa9, 0x1b, 0x3a, 0x8b, 0xd0, 0x49, 0xbf, 0x2a, 0xeb, 0x92, 0x01, 0x42, 0x77, 0x22, 0x22, 0xc9, 0xca}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_5_4_23(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("03f2f331f4142d4f24b43aa10279a89652d4e7537221a1a7b2a25deb551e5de9ac497411c227a94e45f91c2d1c13cc046cf4ce14e32d058734210d44a87ee1b73f"),
			fromBase16("034f090d73b55803030cf0361a5d8081bfb79f851523feac0a2124d08d4013ff08487771a870d0479dc0686c62f7718dfecf024b17c9267678059171339cc00839"),
		},
	}
	msg := []byte{0x32, 0x8c, 0x65, 0x9e, 0x0a, 0x64, 0x37, 0x43, 0x3c, 0xce, 0xb7, 0x3c, 0x14}
	sig := []byte{}
	salt := []byte{0x9a, 0xec, 0x4a, 0x74, 0x80, 0xd5, 0xbb, 0xc4, 0x29, 0x20, 0xd7, 0xca, 0x23, 0x5d, 0xb6, 0x74, 0x98, 0x9c, 0x9a, 0xac}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_5_5_24(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("03f2f331f4142d4f24b43aa10279a89652d4e7537221a1a7b2a25deb551e5de9ac497411c227a94e45f91c2d1c13cc046cf4ce14e32d058734210d44a87ee1b73f"),
			fromBase16("034f090d73b55803030cf0361a5d8081bfb79f851523feac0a2124d08d4013ff08487771a870d0479dc0686c62f7718dfecf024b17c9267678059171339cc00839"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xe2, 0x0c, 0x1e, 0x98, 0x78, 0x51, 0x2c, 0x39, 0x97, 0x0f, 0x58, 0x37, 0x5e, 0x15, 0x49, 0xa6, 0x8b, 0x64, 0xf3, 0x1d}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_6_1_25(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("04f0548c9626ab1ebf1244934741d99a06220efa2a5856aa0e75730b2ec96adc86be894fa2803b53a5e85d276acbd29ab823f80a7391bb54a5051672fb04eeb543"),
			fromBase16("0483e0ae47915587743ff345362b555d3962d98bb6f15f848b4c92b1771ca8ed107d8d3ee65ec44517dd0faa481a387e902f7a2e747c269e7ea44480bc538b8e5b"),
		},
	}
	msg := []byte{0x0a, 0x20, 0xb7, 0x74, 0xad, 0xdc, 0x2f, 0xa5, 0x12, 0x45, 0xed, 0x7c, 0xb9, 0xda, 0x60, 0x9e, 0x50, 0xca, 0xc6, 0x63, 0x6a, 0x52, 0x54, 0x3f, 0x97, 0x45, 0x8e, 0xed, 0x73, 0x40, 0xf8, 0xd5, 0x3f, 0xfc, 0x64, 0x91, 0x8f, 0x94, 0x90, 0x78, 0xee, 0x03, 0xef, 0x60, 0xd4, 0x2b, 0x5f, 0xec, 0x24, 0x60, 0x50, 0xbd, 0x55, 0x05, 0xcd, 0x8c, 0xb5, 0x97, 0xba, 0xd3, 0xc4, 0xe7, 0x13, 0xb0, 0xef, 0x30, 0x64, 0x4e, 0x76, 0xad, 0xab, 0xb0, 0xde, 0x01, 0xa1, 0x56, 0x1e, 0xfb, 0x25, 0x51, 0x58, 0xc7, 0x4f, 0xc8, 0x01, 0xe6, 0xe9, 0x19, 0xe5, 0x81, 0xb4, 0x6f, 0x0f, 0x0d, 0xdd, 0x08, 0xe4, 0xf3, 0x4c, 0x78, 0x10, 0xb5, 0xed, 0x83, 0x18, 0xf9, 0x1d, 0x7c, 0x8c}
	sig := []byte{}
	salt := []byte{0x5b, 0x4e, 0xa2, 0xef, 0x62, 0x9c, 0xc2, 0x2f, 0x3b, 0x53, 0x8e, 0x01, 0x69, 0x04, 0xb4, 0x7b, 0x1e, 0x40, 0xbf, 0xd5}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_6_2_26(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("04f0548c9626ab1ebf1244934741d99a06220efa2a5856aa0e75730b2ec96adc86be894fa2803b53a5e85d276acbd29ab823f80a7391bb54a5051672fb04eeb543"),
			fromBase16("0483e0ae47915587743ff345362b555d3962d98bb6f15f848b4c92b1771ca8ed107d8d3ee65ec44517dd0faa481a387e902f7a2e747c269e7ea44480bc538b8e5b"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x83, 0x14, 0x6a, 0x9e, 0x78, 0x27, 0x22, 0xc2, 0x8b, 0x01, 0x4f, 0x98, 0xb4, 0x26, 0x7b, 0xda, 0x2a, 0xc9, 0x50, 0x4f}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_6_3_27(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("04f0548c9626ab1ebf1244934741d99a06220efa2a5856aa0e75730b2ec96adc86be894fa2803b53a5e85d276acbd29ab823f80a7391bb54a5051672fb04eeb543"),
			fromBase16("0483e0ae47915587743ff345362b555d3962d98bb6f15f848b4c92b1771ca8ed107d8d3ee65ec44517dd0faa481a387e902f7a2e747c269e7ea44480bc538b8e5b"),
		},
	}
	msg := []byte{0x0f, 0x61, 0x95, 0xd0, 0x4a, 0x6e, 0x6f, 0xc7, 0xe2, 0xc9, 0x60, 0x0d, 0xbf, 0x84, 0x0c, 0x39, 0xea, 0x8d, 0x4d, 0x62, 0x4f, 0xd5, 0x35, 0x07, 0x01, 0x6b, 0x0e, 0x26, 0x85, 0x8a, 0x5e, 0x0a, 0xec, 0xd7, 0xad, 0xa5, 0x43, 0xae, 0x5c, 0x0a, 0xb3, 0xa6, 0x25, 0x99, 0xcb, 0xa0, 0xa5, 0x4e, 0x6b, 0xf4, 0x46, 0xe2, 0x62, 0xf9, 0x89, 0x97, 0x8f, 0x9d, 0xdf, 0x5e, 0x9a, 0x41}
	sig := []byte{}
	salt := []byte{0xa8, 0x7b, 0x8a, 0xed, 0x07, 0xd7, 0xb8, 0xe2, 0xda, 0xf1, 0x4d, 0xdc, 0xa4, 0xac, 0x68, 0xc4, 0xd0, 0xaa, 0xbf, 0xf8}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_6_4_28(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("04f0548c9626ab1ebf1244934741d99a06220efa2a5856aa0e75730b2ec96adc86be894fa2803b53a5e85d276acbd29ab823f80a7391bb54a5051672fb04eeb543"),
			fromBase16("0483e0ae47915587743ff345362b555d3962d98bb6f15f848b4c92b1771ca8ed107d8d3ee65ec44517dd0faa481a387e902f7a2e747c269e7ea44480bc538b8e5b"),
		},
	}
	msg := []byte{0x33, 0x7d, 0x25, 0xfe, 0x98, 0x10, 0xeb, 0xca, 0x0d, 0xe4, 0xd4, 0x65, 0x8d, 0x3c, 0xeb, 0x8e, 0x0f, 0xe4, 0xc0, 0x66, 0xab, 0xa3, 0xbc, 0xc4, 0x8b, 0x10, 0x5d, 0x3b, 0xf7, 0xe0, 0x25, 0x7d, 0x44, 0xfe, 0xce, 0xa6, 0x59, 0x6f, 0x4d, 0x0c, 0x59, 0xa0, 0x84, 0x02, 0x83, 0x36, 0x78, 0xf7, 0x06, 0x20, 0xf9, 0x13, 0x8d, 0xfe, 0xb7, 0xde, 0xd9, 0x05, 0xe4, 0xa6, 0xd5, 0xf0, 0x5c, 0x47, 0x3d, 0x55, 0x93, 0x66, 0x52, 0xe2, 0xa5, 0xdf, 0x43, 0xc0, 0xcf, 0xda, 0x7b, 0xac, 0xaf, 0x30, 0x87, 0xf4, 0x52, 0x4b, 0x06, 0xcf, 0x42, 0x15, 0x7d, 0x01, 0x53, 0x97, 0x39, 0xf7, 0xfd, 0xde, 0xc9, 0xd5, 0x81, 0x25, 0xdf, 0x31, 0xa3, 0x2e, 0xab, 0x06, 0xc1, 0x9b, 0x71, 0xf1, 0xd5, 0xbf}
	sig := []byte{}
	salt := []byte{0xa3, 0x79, 0x32, 0xf8, 0xa7, 0x49, 0x4a, 0x94, 0x2d, 0x6f, 0x76, 0x74, 0x38, 0xe7, 0x24, 0xd6, 0xd0, 0xc0, 0xef, 0x18}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_6_5_29(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("04f0548c9626ab1ebf1244934741d99a06220efa2a5856aa0e75730b2ec96adc86be894fa2803b53a5e85d276acbd29ab823f80a7391bb54a5051672fb04eeb543"),
			fromBase16("0483e0ae47915587743ff345362b555d3962d98bb6f15f848b4c92b1771ca8ed107d8d3ee65ec44517dd0faa481a387e902f7a2e747c269e7ea44480bc538b8e5b"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x7b, 0x79, 0x0c, 0x1d, 0x62, 0xf7, 0xb8, 0x4e, 0x94, 0xdf, 0x6a, 0xf2, 0x89, 0x17, 0xcf, 0x57, 0x10, 0x18, 0x11, 0x0e}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_7_1_30(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("07eefb424b0e3a40e4208ee5afb280b22317308114dde0b4b64f730184ec68da6ce2867a9f48ed7726d5e2614ed04a5410736c8c714ee702474298c6292af07535"),
			fromBase16("070830dbf947eac0228de26314b59b66994cc60e8360e75d3876298f8f8a7d141da064e5ca026a973e28f254738cee669c721b034cb5f8e244dadd7cd1e159d547"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xb7, 0x86, 0x7a, 0x59, 0x95, 0x8c, 0xb5, 0x43, 0x28, 0xf8, 0x77, 0x5e, 0x65, 0x46, 0xec, 0x06, 0xd2, 0x7e, 0xaa, 0x50}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_7_2_31(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("07eefb424b0e3a40e4208ee5afb280b22317308114dde0b4b64f730184ec68da6ce2867a9f48ed7726d5e2614ed04a5410736c8c714ee702474298c6292af07535"),
			fromBase16("070830dbf947eac0228de26314b59b66994cc60e8360e75d3876298f8f8a7d141da064e5ca026a973e28f254738cee669c721b034cb5f8e244dadd7cd1e159d547"),
		},
	}
	msg := []byte{0x8d, 0x80, 0xd2, 0xd0, 0x8d, 0xbd, 0x19, 0xc1, 0x54, 0xdf, 0x3f, 0x14, 0x67, 0x3a, 0x14, 0xbd, 0x03, 0x73, 0x52, 0x31, 0xf2, 0x4e, 0x86, 0xbf, 0x15, 0x3d, 0x0e, 0x69, 0xe7, 0x4c, 0xbf, 0xf7, 0xb1, 0x83, 0x6e, 0x66, 0x4d, 0xe8, 0x3f, 0x68, 0x01, 0x24, 0x37, 0x0f, 0xc0, 0xf9, 0x6c, 0x9b, 0x65, 0xc0, 0x7a, 0x36, 0x6b, 0x64, 0x4c, 0x4a, 0xb3}
	sig := []byte{}
	salt := []byte{0x0c, 0x09, 0x58, 0x22, 0x66, 0xdf, 0x08, 0x63, 0x10, 0x82, 0x1b, 0xa7, 0xe1, 0x8d, 0xf6, 0x4d, 0xfe, 0xe6, 0xde, 0x09}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_7_3_32(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("07eefb424b0e3a40e4208ee5afb280b22317308114dde0b4b64f730184ec68da6ce2867a9f48ed7726d5e2614ed04a5410736c8c714ee702474298c6292af07535"),
			fromBase16("070830dbf947eac0228de26314b59b66994cc60e8360e75d3876298f8f8a7d141da064e5ca026a973e28f254738cee669c721b034cb5f8e244dadd7cd1e159d547"),
		},
	}
	msg := []byte{0x80, 0x84, 0x05, 0xcd, 0xfc, 0x1a, 0x58, 0xb9, 0xbb, 0x03, 0x97, 0xc7, 0x20, 0x72, 0x2a, 0x81, 0xff, 0xfb, 0x76, 0x27, 0x8f, 0x33, 0x59, 0x17, 0xef, 0x9c, 0x47, 0x38, 0x14, 0xb3, 0xe0, 0x16, 0xba, 0x29, 0x73, 0xcd, 0x27, 0x65, 0xf8, 0xf3, 0xf8, 0x2d, 0x6c, 0xc3, 0x8a, 0xa7, 0xf8, 0x55, 0x18, 0x27, 0xfe, 0x8d, 0x1e, 0x38, 0x84, 0xb7, 0xe6, 0x1c, 0x94, 0x68, 0x3b, 0x8f, 0x82, 0xf1, 0x84, 0x3b, 0xda, 0xe2, 0x25, 0x7e, 0xee, 0xc9, 0x81, 0x2a, 0xd4, 0xc2, 0xcf, 0x28, 0x3c, 0x34, 0xe0, 0xb0, 0xae, 0x0f, 0xe3, 0xcb, 0x99, 0x0c, 0xf8, 0x8f, 0x2e, 0xf9}
	sig := []byte{}
	salt := []byte{0x28, 0x03, 0x9d, 0xcf, 0xe1, 0x06, 0xd3, 0xb8, 0x29, 0x66, 0x11, 0x25, 0x8c, 0x4a, 0x56, 0x65, 0x1c, 0x9e, 0x92, 0xdd}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_7_4_33(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("07eefb424b0e3a40e4208ee5afb280b22317308114dde0b4b64f730184ec68da6ce2867a9f48ed7726d5e2614ed04a5410736c8c714ee702474298c6292af07535"),
			fromBase16("070830dbf947eac0228de26314b59b66994cc60e8360e75d3876298f8f8a7d141da064e5ca026a973e28f254738cee669c721b034cb5f8e244dadd7cd1e159d547"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xa7, 0x78, 0x21, 0xeb, 0xbb, 0xef, 0x24, 0x62, 0x8e, 0x4e, 0x12, 0xe1, 0xd0, 0xea, 0x96, 0xde, 0x39, 0x8f, 0x7b, 0x0f}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_7_5_34(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("07eefb424b0e3a40e4208ee5afb280b22317308114dde0b4b64f730184ec68da6ce2867a9f48ed7726d5e2614ed04a5410736c8c714ee702474298c6292af07535"),
			fromBase16("070830dbf947eac0228de26314b59b66994cc60e8360e75d3876298f8f8a7d141da064e5ca026a973e28f254738cee669c721b034cb5f8e244dadd7cd1e159d547"),
		},
	}
	msg := []byte{0x45, 0x01, 0x3c, 0xeb, 0xaf, 0xd9, 0x60, 0xb2, 0x55, 0x47, 0x6a, 0x8e, 0x25, 0x98, 0xb9, 0xaa, 0x32, 0xef, 0xbe, 0x6d, 0xc1, 0xf3, 0x4f, 0x4a, 0x49, 0x8d, 0x8c, 0xf5, 0xa2, 0xb4, 0x54, 0x8d, 0x08, 0xc5, 0x5d, 0x5f, 0x95, 0xf7, 0xbc, 0xc9, 0x61, 0x91, 0x63, 0x05, 0x6f, 0x2d, 0x58, 0xb5, 0x2f, 0xa0, 0x32}
	sig := []byte{}
	salt := []byte{0x9d, 0x5a, 0xd8, 0xeb, 0x45, 0x21, 0x34, 0xb6, 0x5d, 0xc3, 0xa9, 0x8b, 0x6a, 0x73, 0xb5, 0xf7, 0x41, 0x60, 0x9c, 0xd6}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_8_1_35(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("08dad7f11363faa623d5d6d5e8a319328d82190d7127d2846c439b0ab72619b0a43a95320e4ec34fc3a9cea876422305bd76c5ba7be9e2f410c8060645a1d29edb"),
			fromBase16("0847e732376fc7900f898ea82eb2b0fc418565fdae62f7d9ec4ce2217b97990dd272db157f99f63c0dcbb9fbacdbd4c4dadb6df67756358ca4174825b48f49706d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x1d, 0x65, 0x49, 0x1d, 0x79, 0xc8, 0x64, 0xb3, 0x73, 0x00, 0x9b, 0xe6, 0xf6, 0xf2, 0x46, 0x7b, 0xac, 0x4c, 0x78, 0xfa}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_8_2_36(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("08dad7f11363faa623d5d6d5e8a319328d82190d7127d2846c439b0ab72619b0a43a95320e4ec34fc3a9cea876422305bd76c5ba7be9e2f410c8060645a1d29edb"),
			fromBase16("0847e732376fc7900f898ea82eb2b0fc418565fdae62f7d9ec4ce2217b97990dd272db157f99f63c0dcbb9fbacdbd4c4dadb6df67756358ca4174825b48f49706d"),
		},
	}
	msg := []byte{0xe2, 0xf9, 0x6e, 0xaf, 0x0e, 0x05, 0xe7, 0xba, 0x32, 0x6e, 0xcc, 0xa0, 0xba, 0x7f, 0xd2, 0xf7, 0xc0, 0x23, 0x56, 0xf3, 0xce, 0xde, 0x9d, 0x0f, 0xaa, 0xbf, 0x4f, 0xcc, 0x8e, 0x60, 0xa9, 0x73, 0xe5, 0x59, 0x5f, 0xd9, 0xea, 0x08}
	sig := []byte{}
	salt := []byte{0x43, 0x5c, 0x09, 0x8a, 0xa9, 0x90, 0x9e, 0xb2, 0x37, 0x7f, 0x12, 0x48, 0xb0, 0x91, 0xb6, 0x89, 0x87, 0xff, 0x18, 0x38}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_8_3_37(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("08dad7f11363faa623d5d6d5e8a319328d82190d7127d2846c439b0ab72619b0a43a95320e4ec34fc3a9cea876422305bd76c5ba7be9e2f410c8060645a1d29edb"),
			fromBase16("0847e732376fc7900f898ea82eb2b0fc418565fdae62f7d9ec4ce2217b97990dd272db157f99f63c0dcbb9fbacdbd4c4dadb6df67756358ca4174825b48f49706d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xc6, 0xeb, 0xbe, 0x76, 0xdf, 0x0c, 0x4a, 0xea, 0x32, 0xc4, 0x74, 0x17, 0x5b, 0x2f, 0x13, 0x68, 0x62, 0xd0, 0x45, 0x29}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_8_4_38(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("08dad7f11363faa623d5d6d5e8a319328d82190d7127d2846c439b0ab72619b0a43a95320e4ec34fc3a9cea876422305bd76c5ba7be9e2f410c8060645a1d29edb"),
			fromBase16("0847e732376fc7900f898ea82eb2b0fc418565fdae62f7d9ec4ce2217b97990dd272db157f99f63c0dcbb9fbacdbd4c4dadb6df67756358ca4174825b48f49706d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x02, 0x1f, 0xdc, 0xc6, 0xeb, 0xb5, 0xe1, 0x9b, 0x1c, 0xb1, 0x6e, 0x9c, 0x67, 0xf2, 0x76, 0x81, 0x65, 0x7f, 0xe2, 0x0a}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_8_5_39(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("08dad7f11363faa623d5d6d5e8a319328d82190d7127d2846c439b0ab72619b0a43a95320e4ec34fc3a9cea876422305bd76c5ba7be9e2f410c8060645a1d29edb"),
			fromBase16("0847e732376fc7900f898ea82eb2b0fc418565fdae62f7d9ec4ce2217b97990dd272db157f99f63c0dcbb9fbacdbd4c4dadb6df67756358ca4174825b48f49706d"),
		},
	}
	msg := []byte{0x04, 0xdc, 0x25, 0x1b, 0xe7, 0x2e, 0x88, 0xe5, 0x72, 0x34, 0x85, 0xb6, 0x38, 0x3a, 0x63, 0x7e, 0x2f, 0xef, 0xe0, 0x76, 0x60, 0xc5, 0x19, 0xa5, 0x60, 0xb8, 0xbc, 0x18, 0xbd, 0xed, 0xb8, 0x6e, 0xae, 0x23, 0x64, 0xea, 0x53, 0xba, 0x9d, 0xca, 0x6e, 0xb3, 0xd2, 0xe7, 0xd6, 0xb8, 0x06, 0xaf, 0x42, 0xb3, 0xe8, 0x7f, 0x29, 0x1b, 0x4a, 0x88, 0x81, 0xd5, 0xbf, 0x57, 0x2c, 0xc9, 0xa8, 0x5e, 0x19, 0xc8, 0x6a, 0xcb, 0x28, 0xf0, 0x98, 0xf9, 0xda, 0x03, 0x83, 0xc5, 0x66, 0xd3, 0xc0, 0xf5, 0x8c, 0xfd, 0x8f, 0x39, 0x5d, 0xcf, 0x60, 0x2e, 0x5c, 0xd4, 0x0e, 0x8c, 0x71, 0x83, 0xf7, 0x14, 0x99, 0x6e, 0x22, 0x97, 0xef}
	sig := []byte{}
	salt := []byte{0xc5, 0x58, 0xd7, 0x16, 0x7c, 0xbb, 0x45, 0x08, 0xad, 0xa0, 0x42, 0x97, 0x1e, 0x71, 0xb1, 0x37, 0x7e, 0xea, 0x42, 0x69}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_9_1_40(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("f8eb97e98df12664eefdb761596a69ddcd0e76daece6ed4bf5a1b50ac086f7928a4d2f8726a77e515b74da41988f220b1cc87aa1fc810ce99a82f2d1ce821edced794c6941f42c7a1a0b8c4d28c75ec60b652279f6154a762aed165d47dee367"),
			fromBase16("ed4d71d0a6e24b93c2e5f6b4bbe05f5fb0afa042d204fe3378d365c2f288b6a8dad7efe45d153eef40cacc7b81ff934002d108994b94a5e4728cd9c963375ae49965bda55cbf0efed8d6553b4027f2d86208a6e6b489c176128092d629e49d3d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xc0, 0xa4, 0x25, 0x31, 0x3d, 0xf8, 0xd7, 0x56, 0x4b, 0xd2, 0x43, 0x4d, 0x31, 0x15, 0x23, 0xd5, 0x25, 0x7e, 0xed, 0x80}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_9_2_41(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("f8eb97e98df12664eefdb761596a69ddcd0e76daece6ed4bf5a1b50ac086f7928a4d2f8726a77e515b74da41988f220b1cc87aa1fc810ce99a82f2d1ce821edced794c6941f42c7a1a0b8c4d28c75ec60b652279f6154a762aed165d47dee367"),
			fromBase16("ed4d71d0a6e24b93c2e5f6b4bbe05f5fb0afa042d204fe3378d365c2f288b6a8dad7efe45d153eef40cacc7b81ff934002d108994b94a5e4728cd9c963375ae49965bda55cbf0efed8d6553b4027f2d86208a6e6b489c176128092d629e49d3d"),
		},
	}
	msg := []byte{0xc8, 0xc9, 0xc6, 0xaf, 0x04, 0xac, 0xda, 0x41, 0x4d, 0x22, 0x7e, 0xf2, 0x3e, 0x08, 0x20, 0xc3, 0x73, 0x2c, 0x50, 0x0d, 0xc8, 0x72, 0x75, 0xe9, 0x5b, 0x0d, 0x09, 0x54, 0x13, 0x99, 0x3c, 0x26, 0x58, 0xbc, 0x1d, 0x98, 0x85, 0x81, 0xba, 0x87, 0x9c, 0x2d, 0x20, 0x1f, 0x14, 0xcb, 0x88, 0xce, 0xd1, 0x53, 0xa0, 0x19, 0x69, 0xa7, 0xbf, 0x0a, 0x7b, 0xe7, 0x9c, 0x84, 0xc1, 0x48, 0x6b, 0xc1, 0x2b, 0x3f, 0xa6, 0xc5, 0x98, 0x71, 0xb6, 0x82, 0x7c, 0x8c, 0xe2, 0x53, 0xca, 0x5f, 0xef, 0xa8, 0xa8, 0xc6, 0x90, 0xbf, 0x32, 0x6e, 0x8e, 0x37, 0xcd, 0xb9, 0x6d, 0x90, 0xa8, 0x2e, 0xba, 0xb6, 0x9f, 0x86, 0x35, 0x0e, 0x18, 0x22, 0xe8, 0xbd, 0x53, 0x6a, 0x2e}
	sig := []byte{}
	salt := []byte{0xb3, 0x07, 0xc4, 0x3b, 0x48, 0x50, 0xa8, 0xda, 0xc2, 0xf1, 0x5f, 0x32, 0xe3, 0x78, 0x39, 0xef, 0x8c, 0x5c, 0x0e, 0x91}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_9_3_42(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("f8eb97e98df12664eefdb761596a69ddcd0e76daece6ed4bf5a1b50ac086f7928a4d2f8726a77e515b74da41988f220b1cc87aa1fc810ce99a82f2d1ce821edced794c6941f42c7a1a0b8c4d28c75ec60b652279f6154a762aed165d47dee367"),
			fromBase16("ed4d71d0a6e24b93c2e5f6b4bbe05f5fb0afa042d204fe3378d365c2f288b6a8dad7efe45d153eef40cacc7b81ff934002d108994b94a5e4728cd9c963375ae49965bda55cbf0efed8d6553b4027f2d86208a6e6b489c176128092d629e49d3d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x9a, 0x2b, 0x00, 0x7e, 0x80, 0x97, 0x8b, 0xbb, 0x19, 0x2c, 0x35, 0x4e, 0xb7, 0xda, 0x9a, 0xed, 0xfc, 0x74, 0xdb, 0xf5}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_9_4_43(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("f8eb97e98df12664eefdb761596a69ddcd0e76daece6ed4bf5a1b50ac086f7928a4d2f8726a77e515b74da41988f220b1cc87aa1fc810ce99a82f2d1ce821edced794c6941f42c7a1a0b8c4d28c75ec60b652279f6154a762aed165d47dee367"),
			fromBase16("ed4d71d0a6e24b93c2e5f6b4bbe05f5fb0afa042d204fe3378d365c2f288b6a8dad7efe45d153eef40cacc7b81ff934002d108994b94a5e4728cd9c963375ae49965bda55cbf0efed8d6553b4027f2d86208a6e6b489c176128092d629e49d3d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x70, 0xf3, 0x82, 0xbd, 0xdf, 0x4d, 0x5d, 0x2d, 0xd8, 0x8b, 0x3b, 0xc7, 0xb7, 0x30, 0x8b, 0xe6, 0x32, 0xb8, 0x40, 0x45}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_9_5_44(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16("f8eb97e98df12664eefdb761596a69ddcd0e76daece6ed4bf5a1b50ac086f7928a4d2f8726a77e515b74da41988f220b1cc87aa1fc810ce99a82f2d1ce821edced794c6941f42c7a1a0b8c4d28c75ec60b652279f6154a762aed165d47dee367"),
			fromBase16("ed4d71d0a6e24b93c2e5f6b4bbe05f5fb0afa042d204fe3378d365c2f288b6a8dad7efe45d153eef40cacc7b81ff934002d108994b94a5e4728cd9c963375ae49965bda55cbf0efed8d6553b4027f2d86208a6e6b489c176128092d629e49d3d"),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0xd6, 0x89, 0x25, 0x7a, 0x86, 0xef, 0xfa, 0x68, 0x21, 0x2c, 0x5e, 0x0c, 0x61, 0x9e, 0xca, 0x29, 0x5f, 0xb9, 0x1b, 0x67}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_10_1_45(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16(""),
			fromBase16(""),
		},
	}
	msg := []byte{0x88, 0x31, 0x77, 0xe5, 0x12, 0x6b, 0x9b, 0xe2, 0xd9, 0xa9, 0x68, 0x03, 0x27, 0xd5, 0x37, 0x0c, 0x6f, 0x26, 0x86, 0x1f, 0x58, 0x20, 0xc4, 0x3d, 0xa6, 0x7a, 0x3a, 0xd6, 0x09}
	sig := []byte{}
	salt := []byte{0x04, 0xe2, 0x15, 0xee, 0x6f, 0xf9, 0x34, 0xb9, 0xda, 0x70, 0xd7, 0x73, 0x0c, 0x87, 0x34, 0xab, 0xfc, 0xec, 0xde, 0x89}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_10_2_46(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16(""),
			fromBase16(""),
		},
	}
	msg := []byte{}
	sig := []byte{}
	salt := []byte{0x8b, 0x2b, 0xdd, 0x4b, 0x40, 0xfa, 0xf5, 0x45, 0xc7, 0x78, 0xdd, 0xf9, 0xbc, 0x1a, 0x49, 0xcb, 0x57, 0xf9, 0xb7, 0x1b}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_10_3_47(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16(""),
			fromBase16(""),
		},
	}
	msg := []byte{0x48, 0xb2, 0xb6, 0xa5, 0x7a, 0x63, 0xc8, 0x4c, 0xea, 0x85, 0x9d, 0x65, 0xc6, 0x68, 0x28, 0x4b, 0x08, 0xd9, 0x6b, 0xdc, 0xaa, 0xbe, 0x25, 0x2d, 0xb0, 0xe4, 0xa9, 0x6c, 0xb1, 0xba, 0xc6, 0x01, 0x93, 0x41, 0xdb, 0x6f, 0xbe, 0xfb, 0x8d, 0x10, 0x6b, 0x0e, 0x90, 0xed, 0xa6, 0xbc, 0xc6, 0xc6, 0x26, 0x2f, 0x37, 0xe7, 0xea, 0x9c, 0x7e, 0x5d, 0x22, 0x6b, 0xd7, 0xdf, 0x85, 0xec, 0x5e, 0x71, 0xef, 0xff, 0x2f, 0x54, 0xc5, 0xdb, 0x57, 0x7f, 0xf7, 0x29, 0xff, 0x91, 0xb8, 0x42, 0x49, 0x1d, 0xe2, 0x74, 0x1d, 0x0c, 0x63, 0x16, 0x07, 0xdf, 0x58, 0x6b, 0x90, 0x5b, 0x23, 0xb9, 0x1a, 0xf1, 0x3d, 0xa1, 0x23, 0x04, 0xbf, 0x83, 0xec, 0xa8, 0xa7, 0x3e, 0x87, 0x1f, 0xf9, 0xdb}
	sig := []byte{}
	salt := []byte{0x4e, 0x96, 0xfc, 0x1b, 0x39, 0x8f, 0x92, 0xb4, 0x46, 0x71, 0x01, 0x0c, 0x0d, 0xc3, 0xef, 0xd6, 0xe2, 0x0c, 0x2d, 0x73}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_10_4_48(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16(""),
			fromBase16(""),
		},
	}
	msg := []byte{0x0b, 0x87, 0x77, 0xc7, 0xf8, 0x39, 0xba, 0xf0, 0xa6, 0x4b, 0xbb, 0xdb, 0xc5, 0xce, 0x79, 0x75, 0x5c, 0x57, 0xa2, 0x05, 0xb8, 0x45, 0xc1, 0x74, 0xe2, 0xd2, 0xe9, 0x05, 0x46, 0xa0, 0x89, 0xc4, 0xe6, 0xec, 0x8a, 0xdf, 0xfa, 0x23, 0xa7, 0xea, 0x97, 0xba, 0xe6, 0xb6, 0x5d, 0x78, 0x2b, 0x82, 0xdb, 0x5d, 0x2b, 0x5a, 0x56, 0xd2, 0x2a, 0x29, 0xa0, 0x5e, 0x7c, 0x44, 0x33, 0xe2, 0xb8, 0x2a, 0x62, 0x1a, 0xbb, 0xa9, 0x0a, 0xdd, 0x05, 0xce, 0x39, 0x3f, 0xc4, 0x8a, 0x84, 0x05, 0x42, 0x45, 0x1a}
	sig := []byte{}
	salt := []byte{0xc7, 0xcd, 0x69, 0x8d, 0x84, 0xb6, 0x51, 0x28, 0xd8, 0x83, 0x5e, 0x3a, 0x8b, 0x1e, 0xb0, 0xe0, 0x1c, 0xb5, 0x41, 0xec}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}

func TestPSSSign_10_5_49(t *testing.T) {
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase16(""),
			E: 0x010001,
		},
		D: fromBase16(""),
		Primes: []*big.Int{
			fromBase16(""),
			fromBase16(""),
		},
	}
	msg := []byte{0xf1, 0x03, 0x6e, 0x00, 0x8e, 0x71, 0xe9, 0x64, 0xda, 0xdc, 0x92, 0x19, 0xed, 0x30, 0xe1, 0x7f, 0x06, 0xb4, 0xb6, 0x8a, 0x95, 0x5c, 0x16, 0xb3, 0x12, 0xb1, 0xed, 0xdf, 0x02, 0x8b, 0x74, 0x97, 0x6b, 0xed, 0x6b, 0x3f, 0x6a, 0x63, 0xd4, 0xe7, 0x78, 0x59, 0x24, 0x3c, 0x9c, 0xcc, 0xdc, 0x98, 0x01, 0x65, 0x23, 0xab, 0xb0, 0x24, 0x83, 0xb3, 0x55, 0x91, 0xc3, 0x3a, 0xad, 0x81, 0x21, 0x3b, 0xb7, 0xc7, 0xbb, 0x1a, 0x47, 0x0a, 0xab, 0xc1, 0x0d, 0x44, 0x25, 0x6c, 0x4d, 0x45, 0x59, 0xd9, 0x16}
	sig := []byte{}
	salt := []byte{0xef, 0xa8, 0xbf, 0xf9, 0x62, 0x12, 0xb2, 0xf4, 0xa3, 0xf3, 0x71, 0xa1, 0x0d, 0x57, 0x41, 0x52, 0x65, 0x5f, 0x5d, 0xfb}

	hash := sha1.New()
	hashed := make([]byte, hash.Size())
	hash.Write(msg)
	hashed = hash.Sum(hashed[:0])

	s, err := SignPSS(rand.Reader, priv, crypto.SHA1, hashed, salt)
	if err != nil {
		t.Error("Error: ", err)
	}
	if !compareBytes(s, sig) {
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM:\n", em, "\n------\n")
		fmt.Print("Siglen = ", len(s))
		fmt.Print("Siglen should be ", len(sig))

		t.Error("Wrong Signature; N.BitLen()", priv.N.BitLen())
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err != nil {
		s := new(big.Int).SetBytes(sig)
		m := encrypt(new(big.Int), &priv.PublicKey, s)
		emBits := priv.N.BitLen() - 1
		emLen := (emBits + 7) / 8
		fmt.Print("EM: (len=", len(m.Bytes()), ")\n", m.Bytes(), "\n------\n")
		em, _ := emsaPSSEncode(hashed, priv.N.BitLen()-1, salt, crypto.SHA1.New())
		fmt.Print("EM should be: (len=", len(em), "; emLen=", emLen, ")\n", em, "\n------\n")
		t.Errorf("Bad Verification")
	}

	// randomly change something
	// We don't need a strong random generator here
	i := weakrand.Intn(len(s))
	s[i] = byte(weakrand.Intn(255))
	if s[i] == sig[i] {
		s[i] = s[i] + 1
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}

	// change back
	s[i] = sig[i]
	// randomly append something
	l := weakrand.Intn(16)
	if l == 0 {
		l = 1
	}
	for i := 0; i < l; i++ {
		s = append(s, byte(weakrand.Intn(255)))
	}
	err = VerifyPSS(&priv.PublicKey, crypto.SHA1, hashed, s, len(salt))
	if err == nil {
		t.Errorf("Bad Verification")
	}
}
